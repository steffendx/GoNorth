...
namespace GoNorth.Services.Export.Dialog.ActionRendering
{
    ...
    public class SetDailyRoutineEventState : BaseActionRenderer<SetDailyRoutineEventState.SetDailyRoutineEventStateData>
    {
        ...
        public override async Task<string> BuildPreviewTextFromParsedData(SetDailyRoutineEventState.SetDailyRoutineEventStateData parsedData, FlexFieldObject flexFieldObject, ExportPlaceholderErrorCollection errorCollection, ExportDialogData child, ExportDialogData parent)
        {
            KortistoNpc eventNpc = await _cachedDbAccess.GetNpcById(parsedData.NpcId);
            if(eventNpc == null)
            {
